...
export function AdminFeedbackDetail() {
  ...
  const handleSendReply = async () => {
    if (!token || !ticket || !ticket.user_id || !replyText.trim()) return;
    setSaving(true);
    setSaveSuccess(false);
    try {
      const updated = await patchAdminFeedback(
      ...
      <div className="space-y-4">
        <div className="bg-white border border-gray-200 rounded-md p-4">
          ...
          <ul className="space-y-3 list-none p-0 m-0">
            <li className="rounded-lg px-3 py-2 bg-slate-100 border border-slate-200 mr-8">
              <p className="text-xs text-slate-500 mb-1">
                Исходное обращение · {new Date(ticket.created_at).toLocaleString("ru-KZ")}
              </p>
            ...
            {(ticket.messages ?? []).map((msg: FeedbackMessageOut) => (
              <li
                key={msg.id}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-white border border-gray-200 rounded-md p-4">
            ...
            <select
              ...
              className="min-h-10 px-3 rounded border border-gray-200 bg-white text-slate-900 w-full"
            >
          ...
          <div className="bg-white border border-gray-200 rounded-md p-4">
            <label htmlFor="admin-feedback-priority-select" className="block font-semibold text-slate-900 mb-2">
              Приоритет
            </label>
            <select
              id="admin-feedback-priority-select"
              value={priority}
          ...
          <div className="bg-white border border-gray-200 rounded-md p-4">
            <label htmlFor="admin-feedback-category-select" className="block font-semibold text-slate-900 mb-2">
              Категория
            </label>
            <select
              id="admin-feedback-category-select"
              value={category}
        ...
        {ticket.user_id ? (
          <div className="bg-white border border-gray-200 rounded-md p-4">
            ...
            {replyTemplates.length > 0 && (
              <div className="mb-2">
                <label htmlFor="admin-reply-template" className="text-sm text-slate-600 mr-2">
                  Вставить шаблон:
                </label>
                <select
                  id="admin-reply-template"
            ...
            <textarea
              ...
              placeholder="Текст ответа… (Ctrl+Enter — отправить)"
            />
            <div className="flex flex-wrap gap-2">
              <Button onClick={handleSaveMetadata} loading={saving}>
                Сохранить
              </Button>
              {replyText.trim() && (
                <Button onClick={handleSendReply} loading={saving} variant="primary">
                  Отправить ответ
                </Button>
              )}
            </div>
