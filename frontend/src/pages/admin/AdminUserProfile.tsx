...
export function AdminUserProfile() {
  ...
  if (error || !user) {
    return (
      <PageLayout>
        ...
        <Link to="/admin/users" className="mt-4 inline-block text-emerald-800 hover:underline">
  ...
  return (
    <PageLayout>
      <div className="mb-4">
        <Link to="/admin/users" className="text-emerald-800 hover:underline text-sm">
          ← К списку пользователей
        </Link>
      </div>
      ...
      <section>
        ...
        {tickets.length === 0 ? (
          ...
          <ul className="list-none p-0 m-0 space-y-1">
            {tickets.map((t) => (
              <li key={t.id}>
                <Link to={`/admin/feedback/${t.id}`} className="text-emerald-800 hover:underline">
                  #{t.id} — {t.subject}
                  {t.status && (
                    <span className="ml-2 text-slate-500 text-sm">
