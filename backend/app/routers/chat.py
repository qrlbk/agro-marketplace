...
def _should_store_history(user: User | None) -> bool:
    return bool(settings.chat_store_enabled and user and getattr(user, "chat_storage_opt_in", False))
...
async def _get_user_context(
    ...
    if allow_region and user.region and str(user.region).strip():
...
async def _prepare_chat_context(
    ...
    allow_personalization = bool(current_user and current_user.chat_storage_opt_in)
    user_context, machine_id = await _get_user_context(db, current_user, allow_personalization)
...
async def chat_message(
    ...
    if _should_store_history(current_user):
...
async def chat_message_stream(
    ...
    async def event_stream():
        ...
        if _should_store_history(current_user) and reply_text:
            await _persist_chat_turn(
                current_user.id,
                body.message or "",
                reply_text,
...
async def chat_list_sessions(
    ...
    if not _should_store_history(current_user):
        return []
    result = await db.execute(
        select(ChatSession)
